...
/**
 * Führt einen JSON-formatierten Ollama-Chat aus, persistiert Prompt + Response,
...
export async function chatJSON<T = unknown>(opts: ChatJSONOptions): Promise<ChatJSONResult<T>> {
  ...
  const baseReq: ChatRequest & { stream: false } = {
    model: opts.model,
    messages,
    stream: false,
    format: "json",
  ...
  try {
    const resp = await ollama.chat({ ...baseReq });
    ...
    try {
      ...
      const repairResp = await ollama.chat({
        ...baseReq,
        messages: [
          ...messages,
      ...
      raw = repairResp.message?.content ?? "";
      if (responsePath) atomicWrite(responsePath, raw);
      parsed = parseRobustJSON<T>(raw);
      ok = true;
    }
